...
class NeuralNetworkVis(Scene):
    def construct(self):
        ...
        config = neat.Config(neat.DefaultGenome, neat.DefaultReproduction,
                         ...
                         config_path)
        with open(".\Fourth model\\best.pickle", "rb") as f:
            winner = pickle.load(f)
        net = NeuralNetworkMobject(winner, config)
        net.label_outputs_text(["Up", "Down", "Left", "Right"])
...
class NeuralNetworkMobject(VGroup):
    CONFIG = {
        ...
        "edge_color": color_gradient([RED_C, GREEN_C], 4),
        "edge_stroke_width": 2,
        "edge_propogation_color": YELLOW,
        "edge_propogation_time": 1,
        "max_shown_neurons": 30,
    ...
    def __init__(self, genome, config, *args, **kwargs):
        VGroup.__init__(self, *args, **kwargs)
        self.connections = [cg.key for cg in genome.connections.values() if cg.enabled]
        self.layers_raw = feed_forward_layers(config.genome_config.input_keys, config.genome_config.output_keys, self.connections)
        #Calculate layers sizes
        layers_c = []
        for layer in self.layers_raw:
            layer_size = 0
            for node in layer:
                if node > 3:
                    layer_size += 1
            if layer_size > 0:
                layers_c.append(layer_size)

        self.layer_sizes = [len(config.genome_config.input_keys), *layers_c, len(config.genome_config.output_keys)]
        self.add_neurons()
        self.add_edges()
    ...
    def get_edge(self, neuron1, neuron2):
        ...
        return Line(
            ...
            neuron2.get_center(),
            buff=self.neuron_radius,
            stroke_color = random.choice(self.edge_color),
            stroke_width=self.edge_stroke_width,
        )
