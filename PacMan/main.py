...
#run NEAT
def run_neat(config):
    #p = neat.Checkpointer.restore_checkpoint('path/to/checkpoint')
    p = neat.Population(config)
    p.add_reporter(neat.StdOutReporter(True))
    stats = neat.StatisticsReporter()
    p.add_reporter(stats)
    p.add_reporter(neat.Checkpointer(generation_interval=10, time_interval_seconds=None, filename_prefix = "checkpoint - "))
...
#obtaint the best genome of a checkpoint
